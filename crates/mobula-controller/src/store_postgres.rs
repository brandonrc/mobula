...
//! `store_sqlite.rs` — same tables, same JSON-text spec columns, same
//! transactional-outbox semantics — with only true dialect adaptations:
//!
//! - Placeholders `$1, $2, …` instead of `?`.
//! - `INTEGER` → `BIGINT` (Postgres `INTEGER` is 32-bit; generations and
//!   unix-second columns are `u64`, bound as `i64` with the same
//!   `.min(i64::MAX as u64)` clamps the SQLite impl uses). `REAL` →
//!   `DOUBLE PRECISION` (Postgres `REAL` is float4; `quantity` is `f64`).
//! - `audit_events.seq INTEGER PRIMARY KEY AUTOINCREMENT` →
//!   `BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY` (standard SQL;
//!   `INSERT … RETURNING seq` is unchanged).
//! - Boolean-as-int columns (`local_users.disabled`, `api_tokens.revoked`)
//!   become real `BOOLEAN` columns with `bool` binds; `SET revoked = 1`
//!   becomes `SET revoked = TRUE`.
//! - Scalar `MAX(a, b)` → `GREATEST(a, b)` in `record_observation` (Postgres
//!   `MAX` is aggregate-only).
//! - SQLite's `BEGIN IMMEDIATE` (serialize writers up front) becomes a real
//!   transaction plus `pg_advisory_xact_lock(hashtext(key))` on the
//!   read-modify-write paths (`upsert_desired`, `upsert_pool`,
//!   `begin_intent`), so two concurrent upserts on the same key serialize
//!   and each sees the other's committed generation bump. Plain
//!   `SELECT … FOR UPDATE` cannot serialize the insert-of-a-new-key race;
//!   the advisory lock covers both.
//! - No `COLUMN_MIGRATIONS` list like SQLite's: Postgres supports
//!   idempotent `ALTER TABLE … ADD COLUMN IF NOT EXISTS`, run in `init` for
//!   deployments created before a column existed (#59's
//!   `audit_events.chain_hash`).
//!
//! The conformance suite in `tests/store.rs` runs against this store when
//! `MOBULA_TEST_POSTGRES_URL` is set (skipped with a message otherwise).
//! To run it locally:
//!
//! ```sh
//! docker run -d --rm --name mobula-pg -p 5432:5432 \
//!   -e POSTGRES_USER=mobula -e POSTGRES_PASSWORD=mobula -e POSTGRES_DB=mobula \
//!   postgres:16-alpine
//! MOBULA_TEST_POSTGRES_URL=postgres://mobula:mobula@localhost:5432/mobula \
//!   cargo test -p mobula-controller --test store
//! ```

use async_trait::async_trait;
use mobula_core::{
    AllocationSpec, ApiTokenRecord, AuditDecision, AuditEvent, AuditFilter, AuditRequired,
    ClusterId, ClusterSpec, ClusterState, DriftCondition, JobRecord, LocalRole, LocalUserRecord,
    PoolSpec,
};
...
/// Same tables as `store_sqlite::SCHEMA`; see the module docs for the dialect
/// adaptations. Idempotent, so `connect` re-applies it on every boot.
const SCHEMA: &str = r#"
...
-- Singleton control flags (e.g. restore quarantine, #41; governance policy).
...
-- actuation. Spec as JSON text; observed_json holds the pool reconcile
-- loop's last ClusterQueue status observation (opaque JSON).
...
-- columns (query-facing, unlike the spec tables). `project = ''` is the
-- pool-level aggregate row; `pool = ''` means the project has no
-- allocation. `source` is 'kueue_ledger' or 'observed_spec' (UsageSource).
...
impl PostgresStore {
    async fn init(pool: &PgPool) -> Result<(), StoreError> {
        // raw_sql uses the simple query protocol: `query()` would prepare
        // the string, and Postgres rejects multi-statement prepared
        // statements ("cannot insert multiple commands").
        sqlx::raw_sql(SCHEMA).execute(pool).await?;
        // #59: additive column for deployments created before the audit
        // chain existed (the CREATE above is a no-op for them).
        sqlx::query(
        ...
        .execute(pool)
        .await?;
        // Truthful Console: additive tombstone-retention anchor for
        // deployments created before it existed (the CREATE above is a no-op
        // for them).
        sqlx::query("ALTER TABLE clusters ADD COLUMN IF NOT EXISTS terminated_at BIGINT")
            .execute(pool)
            .await?;
    ...
    /// Wrap an existing pool (applying the schema to its `search_path`) —
    /// e.g. tests that pin a per-test schema via `after_connect`.
...
/// is never empty.
async fn backfill_audit_chain(pool: &PgPool) -> Result<(), StoreError> {
    loop {
        let row = sqlx::query(
        ...
        sqlx::query("UPDATE audit_events SET chain_hash = $1 WHERE seq = $2")
            .bind(hash)
            .bind(seq as i64)
            .execute(pool)
            .await?;
...
fn row_to_audit_event(row: &PgRow) -> Result<(u64, AuditEvent), StoreError> {
...
fn row_to_cluster(row: PgRow) -> Result<StoredCluster, StoreError> {
...
fn row_to_pool(row: PgRow) -> Result<StoredPool, StoreError> {
    let spec_json: String = row.try_get("spec_json")?;
...
fn row_to_local_user(row: &PgRow) -> Result<LocalUserRecord, StoreError> {
    ...
    Ok(LocalUserRecord {
        ...
        disabled: row.try_get::<bool, _>("disabled")?,
        created_at: row.try_get::<i64, _>("created_at")? as u64,
...
fn row_to_api_token(row: &PgRow) -> Result<ApiTokenRecord, StoreError> {
    Ok(ApiTokenRecord {
...
impl Store for PostgresStore {
    async fn upsert_desired(&self, id: &ClusterId, spec: ClusterSpec) -> Result<u64, StoreError> {
        // BEGIN IMMEDIATE analog (#42): a real transaction plus a per-key
        // advisory lock, so two concurrent upserts on the same id serialize —
        // the second waits, then reads the already-bumped generation instead
        // of both reading gen=N and collapsing two spec changes into one
        // bump. (SELECT … FOR UPDATE alone can't serialize the
        // insert-of-a-new-key race; the advisory lock covers both cases.)
        ...
        let result: Result<u64, StoreError> = async {
            let existing =
                sqlx::query("SELECT spec_json, generation FROM clusters WHERE id = $1")
                    .bind(&id.0)
    ...
    async fn set_desired(&self, id: &ClusterId, desired: DesiredState) -> Result<(), StoreError> {
        ...
        // (Truthful Console); see the SQLite store for the rationale.
    ...
    async fn remove_cluster(&self, id: &ClusterId) -> Result<bool, StoreError> {
        let affected = sqlx::query("DELETE FROM clusters WHERE id = $1")
            .bind(&id.0)
            .execute(&self.pool)
            .await?
            .rows_affected();
    ...
    async fn record_observation(
        ...
        sqlx::query(
            "UPDATE clusters \
             SET observed_state = $1, observed_generation = GREATEST(observed_generation, $2) \
             WHERE id = $3",
        )
        .bind(observed_json)
    ...
    async fn set_condition(
        ...
        sqlx::query("UPDATE clusters SET condition = $1 WHERE id = $2")
            .bind(condition_json)
            .bind(&id.0)
            .execute(&self.pool)
            .await?;
    ...
    async fn set_quarantine(&self, quarantined: bool) -> Result<(), StoreError> {
        sqlx::query(
            "INSERT INTO control (key, value) VALUES ('quarantine', $1) \
    ...
    async fn set_policy(&self, policy: &StoredPolicy) -> Result<(), StoreError> {
        ...
        sqlx::query(
            "INSERT INTO control (key, value) VALUES ('policy', $1) \
             ON CONFLICT(key) DO UPDATE SET value = excluded.value",
        )
        .bind(json)
        .execute(&self.pool)
    ...
    async fn record_attempt(
        ...
        sqlx::query("UPDATE clusters SET failure_count = $1, next_attempt_at = $2 WHERE id = $3")
    ...
    async fn begin_intent(
        ...
        // Atomic open under a transaction + advisory lock so two reconcilers
        // can't both treat the same key as fresh. Insert a pending row if
        // absent; otherwise classify against the stored fingerprint.
        let mut tx = self.pool.begin().await?;
        sqlx::query("SELECT pg_advisory_xact_lock(hashtext($1))")
            .bind(key)
            .execute(&mut *tx)
        ...
        let result: Result<IntentOutcome, StoreError> = async {
            let existing: Option<String> =
                sqlx::query("SELECT params_fingerprint FROM intents WHERE intent_key = $1")
                    .bind(key)
                    .fetch_optional(&mut *tx)
                    .await?
    ...
    async fn complete_intent(&self, key: &str, response_json: &str) -> Result<(), StoreError> {
        sqlx::query(
            "UPDATE intents SET status = 'applied', response_json = $1, completed_at = $2 \
             WHERE intent_key = $3",
        )
        .bind(response_json)
        .bind(now_unix() as i64)
    ...
    async fn get_intent(&self, key: &str) -> Result<Option<IntentRecord>, StoreError> {
        let row = sqlx::query("SELECT * FROM intents WHERE intent_key = $1")
            .bind(key)
            .fetch_optional(&self.pool)
            .await?;
        row.map(|row| {
    ...
    async fn upsert_pool(&self, name: &str, spec: PoolSpec) -> Result<u64, StoreError> {
        // Same transaction + advisory-lock discipline as upsert_desired
        // (#42): two concurrent pool updates serialize, each seeing the
        // other's bump.
        let mut tx = self.pool.begin().await?;
        sqlx::query("SELECT pg_advisory_xact_lock(hashtext($1))")
            .bind(format!("pool/{name}"))
            .execute(&mut *tx)
            .await?;

        let result: Result<u64, StoreError> = async {
            let existing = sqlx::query("SELECT spec_json, generation FROM pools WHERE name = $1")
                .bind(name)
                .fetch_optional(&mut *tx)
                .await?;
    ...
    async fn get_pool(&self, name: &str) -> Result<Option<StoredPool>, StoreError> {
        let row = sqlx::query("SELECT * FROM pools WHERE name = $1")
            .bind(name)
            .fetch_optional(&self.pool)
            .await?;
    ...
    async fn delete_pool(&self, name: &str) -> Result<(), StoreError> {
        let affected = sqlx::query("DELETE FROM pools WHERE name = $1")
            .bind(name)
            .execute(&self.pool)
            .await?
            .rows_affected();
    ...
    async fn record_pool_observation(
        ...
        sqlx::query("UPDATE pools SET observed_json = $1, observed_at = $2 WHERE name = $3")
            .bind(observed_json)
            .bind(now_unix() as i64)
            .bind(name)
            .execute(&self.pool)
    ...
    async fn list_allocations(&self, pool: &str) -> Result<Vec<AllocationSpec>, StoreError> {
        let rows = sqlx::query("SELECT spec_json FROM allocations WHERE pool = $1")
            .bind(pool)
            .fetch_all(&self.pool)
            .await?;
        rows.into_iter()
    ...
    async fn delete_allocation(&self, pool: &str, project: &str) -> Result<(), StoreError> {
        let affected = sqlx::query("DELETE FROM allocations WHERE pool = $1 AND project = $2")
            .bind(pool)
            .bind(project)
            .execute(&self.pool)
            .await?
    ...
    async fn usage_samples(
        ...
        let rows = sqlx::query(
            "SELECT ts, project, pool, resource, quantity, source FROM usage_samples \
             WHERE ts >= $1 AND ts <= $2 \
             AND ($3 IS NULL OR project = $4) \
             AND ($5 IS NULL OR pool = $6) \
             ORDER BY ts ASC",
        )
    ...
    async fn record_audit(&self, event: &AuditEvent) -> Result<u64, StoreError> {
        ...
        // #59: the new row chains from the newest existing row. The
        // transaction-scoped advisory lock serializes read-compute-insert
        // across connections (same pattern as upsert_desired's lock).
        let mut tx = self.pool.begin().await?;
        ...
        // RETURNING is standard SQL — one round trip, no
        // last_insert_rowid() coupling to the connection.
        let row = sqlx::query(
    ...
    async fn list_audit(
        ...
        // conditional appends uniform; `substr(path, 1, length($n)) = $m`
        ...
        if let Some(prefix) = &filter.path_prefix {
            // `push_bind` appends the placeholder itself; never write `$n`.
    ...
    async fn get_local_user(&self, username: &str) -> Result<Option<LocalUserRecord>, StoreError> {
        let row = sqlx::query("SELECT * FROM local_users WHERE username = $1")
            .bind(username)
            .fetch_optional(&self.pool)
            .await?;
    ...
    async fn set_local_user_password(
        ...
        let affected = sqlx::query("UPDATE local_users SET password_hash = $1 WHERE username = $2")
            .bind(password_hash)
            .bind(username)
            .execute(&self.pool)
            .await?
    ...
    async fn set_local_user_role(&self, username: &str, role: LocalRole) -> Result<(), StoreError> {
        let affected = sqlx::query("UPDATE local_users SET role = $1 WHERE username = $2")
            .bind(role.as_str())
            .bind(username)
            .execute(&self.pool)
            .await?
    ...
    async fn set_local_user_disabled(
        ...
        let affected = sqlx::query("UPDATE local_users SET disabled = $1 WHERE username = $2")
            .bind(disabled)
            .bind(username)
            .execute(&self.pool)
            .await?
    ...
    async fn get_api_token_by_prefix(
        ...
        let row = sqlx::query("SELECT * FROM api_tokens WHERE prefix = $1")
            .bind(prefix)
            .fetch_optional(&self.pool)
            .await?;
    ...
    async fn list_api_tokens(&self, username: &str) -> Result<Vec<ApiTokenRecord>, StoreError> {
        let rows =
            sqlx::query("SELECT * FROM api_tokens WHERE username = $1 ORDER BY created_at DESC")
                .bind(username)
                .fetch_all(&self.pool)
                .await?;
    ...
    async fn revoke_api_token(&self, prefix: &str, username: &str) -> Result<(), StoreError> {
        ...
        let affected =
            sqlx::query("UPDATE api_tokens SET revoked = TRUE WHERE prefix = $1 AND username = $2")
                .bind(prefix)
                .bind(username)
                .execute(&self.pool)
                .await?
    ...
    async fn touch_api_token(&self, prefix: &str, now: u64) -> Result<(), StoreError> {
        sqlx::query("UPDATE api_tokens SET last_used_at = $1 WHERE prefix = $2")
    ...
    async fn upsert_role_assignment(
        ...
        // triple's PK), matching the SQLite/in-memory impls.
        sqlx::query(
    ...
    async fn delete_role_assignment(
        ...
        let affected = sqlx::query(
            "DELETE FROM role_assignments WHERE principal = $1 AND role = $2 AND scope = $3",
        )
        .bind(principal)
        .bind(role)
        .bind(scope)
